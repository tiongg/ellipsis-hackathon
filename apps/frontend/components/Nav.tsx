...
import Logo  from './Logo'; // Make sure this component is correctly implemented
import { useState } from 'react';
import { useRouter } from 'next/navigation';
import Link from 'next/link';
import { useAuth } from '../providers/AuthProviders'; // Make sure this hook is correctly implemented
...
export default function Navbar() {
  ...
  const [items, setItems] = useState([]); // Assuming this will be managed elsewhere in your app
  const [isLoading, setIsLoading] = useState(false); // Implement loading state as needed
    ...
    <header className='sticky w-full text-lg top-0 bg-white z-20 py-4 border-b shadow-sm border-gray-100'>
      <div className='container flex justify-between items-center'>
        ...
        <button
          ...
          className='ml-4 bg-blue-500 text-white p-2 px-4 rounded-md items-center gap-2 hidden md:flex'
        >
          {self ? 'Logout' : 'Login'}
        </button>
        ...
        {isMenuOpen && (
          <div className='shadow-lg transition-all fixed top-full right-0 bg-white h-screen p-4 px-8 md:hidden'>
            ...
            <button
              onClick={() => router.push(self ? '/' : '/auth/login')}
              className='ml-4 bg-blue-400 text-white p-2 px-4 rounded-md items-center gap-2'
            >
              {self ? 'Logout' : 'Login'}
            </button>
          </div>
